...
**Resolved 2026-09-03 (was 🔴 CRITICAL as of 2026-08-31, this section was stale until 2026-09-07)**: the `PdfCertification` table was missing in production on 2026-08-31, confirmed broken (`POST /api/reports/verify` always 500 for real PDF uploads). Fixed by applying the table manually (`CREATE TABLE IF NOT EXISTS`, same SQL as `prisma/migrations/20260517000000_init_pdf_certification/migration.sql`, run directly against production Postgres — **not** via `prisma migrate deploy`) and wiring `app/api/reports/generate/route.ts` to sign + persist a certification after PDF generation. A separate third-party bug in the PDF engine blocked real end-to-end proof until it was replaced with a Puppeteer-based renderer, also 2026-09-03. Confirmed live with real data: the table has 6 real certification rows as of 2026-09-07. `_prisma_migrations` still does not exist and never will via this path — that specific claim from the original finding remains true, but is no longer evidence that the feature is broken, since the table was created outside Prisma's own migration tracking. See `STATUS_PROJETO.md` for the full history.
...
- **Trust graph** (`packages/proof-engine/trust-graph/`, moved 2026-09-06 from root `lib/trust-graph/` as part of the Etapa 2 extraction — import via the `@proof-engine/trust-graph/...` alias, not `@/lib/trust-graph/...`, which no longer resolves to anything) — analyzes relationships between screens, campaigns, advertisers, and operators to detect fraud clusters. Nodes score from 0–100; below 40 = `HIGH_RISK`. `src/lib/trust-graph/` is a second, unreachable implementation with the same filenames, unaffected by the move — confirmed dead 2026-09-06, see `src/lib/trust-graph/README.md`.
...
Before calling any real production route/cron job with live test data (via curl, Puppeteer, or otherwise), check whether that call path can trigger a real **external, irreversible side effect** — not just a database row. This includes, but is not limited to: a message to a real recipient (WhatsApp via `sendWhatsApp`, whether `lib/whatsapp.ts` or one of its per-route duplicates; email via Resend; any other channel), a real charge/transfer (Asaas), a real blockchain transaction (Polygon anchor), or a real upload/delete against external storage (R2/S3). If the call path can trigger any of these, either use test data with a fake/non-real value for that specific field (contact, amount, file key, etc.), or get explicit confirmation first — validating that the business logic is correct (status updates, DB rows) is never the same check as confirming no unwanted external effect happened. This rule existed only as a verbally-agreed convention before 2026-09-08 (never written here), which is why a real test of `POST /api/cron/network-partnerships-timeout` sent an unauthorized real WhatsApp message to a real client (`BARBE332`) that day — see `STATUS_PROJETO.md` for the full incident writeup. Don't let this happen again by relying on conversation memory alone.
