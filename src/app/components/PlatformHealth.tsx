    ...
    <div className="min-h-screen" style={{ background: T.bg, color: T.text, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="p-5 rounded-2xl border flex items-center gap-5"
          style={{ background: overallCfg.color + "10", borderColor: overallCfg.color + "30" }}>
          ...
          <div className="flex-1">
            <h2 className="font-black text-xl" style={{ color: overallCfg.color }}>
              {overallStatus === "operational" ? "Todos os sistemas operacionais" :
               overallStatus === "degraded" ? "Desempenho degradado em alguns serviços" : "Interrupção em progresso"}
            </h2>
            <p className="text-sm" style={{ color: T.textSub }}>
              {operationalCount} operacionais · {degradedCount} degradados · {outageCount} fora do ar · Uptime médio 90d: {avgUptime}%
            </p>
          </div>
                  ...
                  {groupServices.map((s, i) => {
                    ...
                    return (
                      <div key={s.name}
                        ...
                        style={{ borderTop: `1px solid ${T.border}`, background: s.status !== "operational" ? cfg.color + "05" : "transparent" }}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-0.5">
                            {bars.slice(0, 60).map((b, bi) => (
                              <div key={bi} className="w-1 h-3 rounded-sm"
                                style={{ background: b === "ok" ? T.success + "60" : cfg.color }} />
                            ))}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-full font-bold"
                          style={{ background: ist.color + "20", color: ist.color }}>
                          {ist.label}
                        </span>
                        <span className="text-xs px-2 py-0.5 rounded-full font-bold"
                      ...
                      <div className="text-sm" style={{ color: T.textSub }}>
                        {inc.id} · {inc.service} · Iniciado: {inc.started}
                      </div>
                    </div>
                  </div>
                  {inc.status !== "resolved" && (
                    <div className="p-3 rounded-xl text-sm" style={{ background: T.panel, color: T.textSub }}>
                      Estamos investigando latência elevada nas chamadas ao nó Polygon RPC. Transações ainda funcionam, mas com atraso. Atualização em 30min.
                    </div>
                  )}
                  {inc.status === "resolved" && (
                    <div className="flex items-center gap-1.5 text-sm" style={{ color: T.success }}>
                      <CheckCircle size={14} />
                      Resolvido sem impacto nos usuários finais.
                    </div>
                  )}
                </div>
              );
          ...
          <div className="space-y-5">
            ...
            <div className="p-5 rounded-2xl border" style={{ background: T.card, borderColor: T.border }}>
              ...
              <ResponsiveContainer width="100%" height={160}>
                <AreaChart data={UP_HISTORY}>
                  <defs>
                    <linearGradient key="ph-lat" id="ph-lat" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={T.primary} stopOpacity={0.35} />
                      <stop offset="95%" stopColor={T.primary} stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [`${v}ms`, "p99"]} />
                  <Area key="area-latency" type="monotone" dataKey="latency" stroke={T.primary} fill="url(#ph-lat)" strokeWidth={2} dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="p-5 rounded-2xl border" style={{ background: T.card, borderColor: T.border }}>
              ...
              <ResponsiveContainer width="100%" height={120}>
                <AreaChart data={UP_HISTORY}>
                  <defs>
                    <linearGradient key="ph-req" id="ph-req" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area key="area-requests" type="monotone" dataKey="requests" stroke={T.success} fill="url(#ph-req)" strokeWidth={2} dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
