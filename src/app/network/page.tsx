...
function isSuccessResponse(data: unknown): data is TrustGraphRouteSuccess {
  if (!data || typeof data !== "object") {
    return false
  }

  const candidate = data as Partial<TrustGraphRouteSuccess>
...
function getErrorMessage(
  data: TrustGraphRouteResponse | null,
  fallback: string
): string {
  if (
    data &&
...
export default function Page() {
  ...
  const loadGraph = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      let data: TrustGraphRouteResponse | null = null

      try {
        data = (await res.json()) as TrustGraphRouteResponse
      } catch {
        data = null
      }
