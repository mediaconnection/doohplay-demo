...
- 🔴 **Contenção ampla na instância do Postgres, ainda não diagnosticada de verdade** — `Query read timeout`/`Connection terminated` continuam acontecendo em `player/event`, `player/heartbeat` e `playlist GET` mesmo depois do fix de `event_chain` (confirmado com logs reais até `17:33:03` de 02/09/2026, depois do deploy). Como `playlist GET`/`heartbeat` não tocam `event_chain`, a causa não é só essa query — é algo mais amplo na instância (recursos, outro processo concorrente, ou algo do lado da infraestrutura do Supabase). Precisa de investigação própria, mais funda, antes de considerar resolvido.
- Criar manutenção de partições de `event_chain` (job de criação automática mensal, ou processo manual) — parado desde 04/2026. Continua uma causa de fundo real (a partição "default" só cresce), mas não explica sozinha o item acima.
