...
- 🟡 **Decisão de infraestrutura pendente**: aumentar o tier de computação do Supabase (`work_mem` de ~2MB é a causa raiz da contenção ampla — ver achado acima). Mitigações gratuitas já aplicadas (fix de `event_chain`, redução do polling de `dashboard_kpis`) reduzem a exposição, mas não resolvem o `work_mem` pequeno em si. Decisão de custo, do usuário — observar o efeito das mitigações antes de decidir.
...
- 🟡 **Backfill de `event_chain_default` (maio-setembro/2026) — avaliado em detalhe (2026-09-02), execução deixada pra próxima sessão de propósito ("cabeça fresca").** Volume real por mês, confirmado: maio 5.640 linhas/6,6MB, junho 223/79KB, julho 20.777/33MB, agosto 56.967/105MB, setembro (em andamento, excluir do escopo) 5.587/3,3MB. **Escopo recomendado: só maio-agosto (83.607 linhas, ~145MB)** — setembro ainda recebe escrita real ao vivo (`player/event`), e qualquer abordagem que destaque a partição `default` (mesmo com `DETACH ... CONCURRENTLY`, suportado no Postgres 17.6 confirmado nesta instância) cria uma janela onde `event_chain` fica sem partição-padrão, e qualquer INSERT de setembro nessa janela falharia com erro de particionamento — risco real de indisponibilidade momentânea, não vale a pena pra dado histórico.
  - **Abordagem mais segura já desenhada, evita destacar/reanexar o `default`**: (1) copiar linhas de maio-agosto pra tabela de staging (`CREATE TABLE staging AS SELECT ... WHERE occurred_at < '2026-09-01'`); (2) `DELETE` dessas mesmas linhas do `event_chain_default` real; (3) com o `default` sem conflito pra maio-agosto, criar as partições `event_chain_2026_05` a `_08` normalmente, sem precisar destacar nada; (4) `INSERT INTO event_chain SELECT * FROM staging` (roteia automático pelas novas partições); (5) apagar a staging.
  - **Achado que precisa ser resolvido antes de executar**: `event_chain_default` tem 14 índices, mas **nenhum em `occurred_at` sozinho** (a própria chave de particionamento) — só aparece dentro de um índice composto começando por `event_id`, que não ajuda um filtro só por `occurred_at`. O `DELETE`/`SELECT` do passo 2-3 forçaria varredura sequencial das ~89 mil linhas — mesma classe de gargalo já corrigida no `event_chain` principal, numa escala menor (89 mil vs 583 mil de `display_events`), mas não é de graça. Considerar criar um índice em `occurred_at` na `default` antes, ou rodar `EXPLAIN ANALYZE` primeiro pra medir o custo real.
