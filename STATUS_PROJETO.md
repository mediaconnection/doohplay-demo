...
- 🟡 **Pendência real, mais funda do que parecia (2026-09-03)**: `valid:false` na verificação de assinatura persiste mesmo depois do fix de prioridade em `signHash()` (commit `92edaee`, correto por si só). Nenhuma das 2 chaves privadas conhecidas (`/etc/secrets/private.pem`, `PRIVATE_PEM`) corresponde a `keys/public.pem` — confirmado via diagnóstico real, duas rodadas. Não se sabe hoje qual é a chave privada correta, se alguma. Decisão pendente do usuário: gerar par de chaves novo e sincronizado é o caminho mais provável. Ver seção acima.
