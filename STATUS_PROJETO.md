...
**Resta, não iniciado**: as demais tabelas do Tier 3 (operacional/negócio) continuam com RLS desabilitado — decisão consciente de não mexer ainda, já que várias fazem parte do desenho público do produto de prova/verificação e precisam de política pensada, não só ativação cega. Nenhuma política de RLS foi criada pras 29 já corrigidas — ficam com acesso zero pra `anon`/`authenticated` até decisão específica de cada política. Achado sinalizado, não decidido: os 3 registros expostos de `financial_closure_signatures` podem justificar tratamento como incidente de segurança formal (rotação de algo, notificação) — decisão do fundador, não presumida aqui.
