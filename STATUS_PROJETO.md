...
**Hipótese com evidência direta, ainda não confirmada**: `git log` em `keys/public.pem` mostra o último commit (`f699314`, 31/05/2026) chamado *"fix: public.pem correto correspondente ao `PRIVATE_PEM` do doohplay-demo"* — ou seja, a chave pública versionada foi deliberadamente ajustada pra bater com a variável de ambiente `PRIVATE_PEM`. Mas `signHash()` (`src/services/pdf/pdfSigner.ts`) procura a chave privada nesta ordem: `/etc/secrets/private.pem` → `keys/private.pem` → `PRIVATE_PEM` (só por último, se os dois primeiros não existirem). Como `/etc/secrets/` neste serviço já guarda pelo menos `certificado-a1.pfx` (achado de sessão anterior), é plausível que `/etc/secrets/private.pem` também exista e `signHash()` esteja assinando com uma chave diferente da que `keys/public.pem` corresponde — nunca chegando a usar o `PRIVATE_PEM` certo. **Não confirmado ainda** — próximo passo é checar o que existe de fato em `/etc/secrets/` no Render antes de mexer em qualquer coisa.
...
- 🟡 **Achado novo (2026-09-03)**: verificação de assinatura RSA retorna `valid:false` no primeiro teste real de ponta a ponta — hipótese não confirmada de que `signHash()` está pegando a chave errada em `/etc/secrets/private.pem` em vez do `PRIVATE_PEM` que `keys/public.pem` foi corrigido pra bater (commit `f699314`). Investigação em andamento, ver seção acima.
