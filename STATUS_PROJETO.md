...
`/etc/secrets/private.pem` **existe** em produção neste serviço (provavelmente sobra de outra finalidade — `/etc/secrets/` aqui também guarda `certificado-a1.pfx`, achado de sessão anterior). `PRIVATE_PEM` **também está configurada**. `signHash()` (`src/services/pdf/pdfSigner.ts`) segue sua ordem de prioridade e usa `/etc/secrets/private.pem` — nunca chega a usar `PRIVATE_PEM`, que é a chave que `keys/public.pem` foi deliberadamente corrigida pra bater (commit `f699314`, 31/05/2026, *"fix: public.pem correto correspondente ao `PRIVATE_PEM` do doohplay-demo"*). A chave pública derivada de `/etc/secrets/private.pem` **não bate** com `keys/public.pem` — explica exatamente o `valid:false` do teste de ponta a ponta acima. Fix ainda não aplicado, aguardando decisão do usuário sobre a causa raiz de `/etc/secrets/private.pem` existir (pode ser proposital pra outra finalidade, não necessariamente lixo).
...
- 🟡 **Achado confirmado (2026-09-03)**: `signHash()` assina com `/etc/secrets/private.pem` (existe em produção, prioridade mais alta na ordem de resolução) em vez de `PRIVATE_PEM` (também configurada, é a que `keys/public.pem` foi corrigida pra bater) — `keysMatch:false` confirmado via diagnóstico real. Explica o `valid:false` do teste de ponta a ponta. Fix não aplicado ainda — decisão pendente: reordenar a prioridade em `signHash()`, ou investigar por que `/etc/secrets/private.pem` existe (pode ter outra finalidade, não necessariamente é lixo). Ver seção acima.
