...
- 🟡 **Decisão de infraestrutura pendente (Upstash)**: rate-limit do Redis segue ativo (confirmado 2026-09-03, mesmo com o fix de idempotência de 27/08 em vigor — a própria leitura `getRepeatableJobs()` já é rejeitada). O churn de restart por commit não relacionado foi mitigado (Build Filters do `doohplay-workers`, ver achado acima), mas o rate-limit em si segue sem solução. Duas opções levantadas, nenhuma decidida: (a) consolidar as 14 conexões Redis distintas do código num cliente só; (b) upgrade do plano Upstash. Decisão do usuário, sem mais pressa agora que o churn parou.
