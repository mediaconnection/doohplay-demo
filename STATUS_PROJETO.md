...
**Honestidade sobre o resultado**: no momento do deploy, presenciei ao vivo um período de distress agudo na instância (respostas de 8-15s, alguns `500`, minhas próprias queries de diagnóstico timeando) que **não desapareceu instantaneamente** com o fix — levou alguns minutos até a instância normalizar sozinha (últimas 10 chamadas de `/api/player/event` monitoradas: todas `200`, 191-360ms). Não dá pra afirmar com certeza se esse pico específico foi coincidência de timing, resíduo de conexões antigas (query em voo antes do deploy) ou parcialmente amplificado pela própria investigação (muitas queries pesadas de diagnóstico em sequência). A correção do plano de execução em si é real e permanente, verificada via `EXPLAIN ANALYZE`, independente desse episódio.

**Achado estrutural separado, maior, não resolvido**: falta manutenção de partições de `event_chain` desde 04/2026 (nenhum job cria as partições mensais seguintes). Isso é a causa de fundo que deixa esse tipo de contenção acontecer de novo — sem isso, a partição "default" só vai continuar crescendo. Registrado como pendência própria abaixo.

## Próximos passos em aberto

- Criar manutenção de partições de `event_chain` (job de criação automática mensal, ou processo manual) — parado desde 04/2026, causa estrutural do achado acima. Sem isso, novos incidentes de contenção são só questão de tempo.
