...
- Investigar `app/api/verify/[hash]/route.ts` (e possivelmente `app/api/events/route.ts`, mesmo padrão) devolvendo `500` cru pro usuário público quando o enqueue falha por rate-limit do Upstash, em vez de degradar com uma mensagem clara de indisponibilidade temporária — achado durante a investigação do circuit-breaker acima, front `app/`, fora do escopo daquela correção.
