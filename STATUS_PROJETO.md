...
### Hipótese em aberto: os 4 widgets podem voltar a mostrar dado assim que o Redis/pipeline de prova for resolvido

`dashboard_kpis` e as outras 3 funções leem de `play_logs_certified`, tabela que está genuinely vazia/quase vazia em produção. Hipótese levantada pelo usuário: como o pipeline real de prova (`runProofChainAggregator()`, ver achado de 2026-08-26 acima) está parado desde 2026-07-24 por rate-limit do Redis (Upstash), é possível que `play_logs_certified` dependa, direta ou indiretamente, desse mesmo pipeline parado — e que os 4 widgets voltem a mostrar dado real assim que o Redis for resolvido e o pipeline retomar.

**Não consegui confirmar essa conexão com certeza**: busquei em todo o código TypeScript do repo (`lib/`, `app/`, scripts) por qualquer `INSERT INTO play_logs_certified` ou equivalente e não encontrei nenhum ponto de escrita — só leituras (`app/api/finance/close-month`, `app/api/reports/campaigns*`, `app/api/proof/impressions/*`, `app/api/impression/*`). Isso significa que a tabela é populada por algo fora do código TS deste repo — trigger/função no Postgres, Edge Function do Supabase, ou escrita direta do player Android (front separado, fora deste repositório). A hipótese é plausível (a tabela claramente pertence ao domínio de prova/certificação, usada ao lado de `event_chain`/`event_blocks`/`certifications`) mas fica **não confirmada** — vale investigar o mecanismo real de escrita antes de prometer que resolver o Redis vai popular o dashboard.
