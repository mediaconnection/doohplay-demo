...
- Mover retroativamente os ~89 mil registros de maio-setembro/2026 de `event_chain_default` pras partições mensais corretas — separado da manutenção acima (já resolvida). Maior risco: requer criar as partições retroativas + realocar dado de uma tabela com escrita concorrente ativa. Não avaliado/decidido ainda.
