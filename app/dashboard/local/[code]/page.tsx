...
function TabConfiguracoes({ code }: { code: string }) {
  const [form, setForm] = useState({
    name: "", business_type: "", address: "", city: "",
    phone: "", email: "", cpf_cnpj: "",
    notif_whatsapp: true, notif_email: false,
  })
  ...
  useEffect(() => {
    fetch(`/api/client/settings/${code}`)
      .then(r => r.json())
      .then(d => {
        setForm({
          name:            d.name            ?? "",
          ...
          email:           d.email           ?? "",
          cpf_cnpj:        d.cpf_cnpj        ?? "",
          notif_whatsapp:  d.notif_whatsapp  ?? true,
          notif_email:     d.notif_email     ?? false,
        })
      })
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch(`/api/client/settings/${code}`, {
        method: "PATCH",
  ...
  if (loading) return (
    <div style={{ padding: 40, textAlign: "center", color: C.text3, fontSize: 14 }}>Carregando…</div>
  )
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px", marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div style={{ gridColumn: "1 / -1" }}>
            ...
            <input style={inputStyle} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Nome do seu negócio" />
          </div>
          <div>
            <label style={labelStyle}>Tipo</label>
            <input style={inputStyle} value={form.business_type} onChange={e => setForm(f => ({ ...f, business_type: e.target.value }))} placeholder="Ex: Barbearia, Padaria…" />
          </div>
          <div>
            <label style={labelStyle}>Cidade</label>
            <input style={inputStyle} value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))} placeholder="São Paulo" />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Endereço</label>
            <input style={inputStyle} value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="Rua, número, bairro" />
          </div>
        </div>
      </div>
      ...
      {/* Contato */}
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px", marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: C.text, marginBottom: 20 }}>Contato & acesso</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div>
            <label style={labelStyle}>WhatsApp (com DDD) *</label>
            <input style={inputStyle} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value.replace(/\D/g, "") }))} placeholder="11 99999-9999" type="tel" />
          </div>
          <div>
            <label style={labelStyle}>Email</label>
            <input style={inputStyle} value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="seu@email.com" type="email" />
          </div>
          <div>
            <label style={labelStyle}>CPF / CNPJ</label>
            <input style={inputStyle} value={form.cpf_cnpj} onChange={e => setForm(f => ({ ...f, cpf_cnpj: e.target.value }))} placeholder="000.000.000-00" />
          </div>
          <div style={{ display: "flex", alignItems: "flex-end" }}>
            <div style={{ fontSize: 11, color: C.text3, lineHeight: 1.5 }}>
              Código do estabelecimento:<br />
              <strong style={{ color: C.text, fontSize: 13, fontFamily: "monospace" }}>{code}</strong>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px", marginBottom: 20 }}>
        ...
        {[
          { key: "notif_whatsapp", label: "WhatsApp", desc: "Receba relatórios mensais e alertas pelo WhatsApp" },
          { key: "notif_email",    label: "Email",    desc: "Receba relatórios e notificações por email" },
        ].map(n => (
          <div key={n.key} onClick={() => setForm(f => ({ ...f, [n.key]: !(f as any)[n.key] }))}
            style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: n.key === "notif_whatsapp" ? `1px solid ${C.border2}` : "none", cursor: "pointer" }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 500, color: C.text }}>{n.label}</div>
              <div style={{ fontSize: 12, color: C.text3, marginTop: 2 }}>{n.desc}</div>
            </div>
            <div style={{
              width: 44, height: 24, borderRadius: 12, transition: "background .2s", flexShrink: 0,
              background: (form as any)[n.key] ? C.blue : C.gray200,
              position: "relative",
            }}>
              <div style={{
      ...
      {success && (
        <div style={{ background: C.greenLt, border: `1px solid ${C.greenBd}`, borderRadius: 8, padding: "10px 14px", marginBottom: 14, fontSize: 13, color: C.green }}>
          ✓ Configurações salvas com sucesso!
        </div>
      )}

      <button onClick={save} disabled={saving} style={{ background: saving ? C.gray300 : C.blue, color: C.white, border: "none", borderRadius: 10, padding: "12px 28px", fontSize: 14, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer" }}>
