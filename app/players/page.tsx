...
type PlayerCampaignRow = {
  ...
  campaign_risk: string | null
  total_events: number
  invalid_events: number
  last_event_at: string | null
}

type CampaignViewRow = PlayerCampaignRow & {
  normalized_risk: "SAFE" | "WATCH" | "HIGH_RISK"
  normalized_score: number
}

function normalizeRisk(
  value: string | null | undefined
): "SAFE" | "WATCH" | "HIGH_RISK" {
...
function shortHash(value: string | null, size = 10): string {
  if (!value) return "—"
...
export default async function PlayerDetailsPage({
    ...
    pool.query(
      ...
      campaign_graph as (
        select
          ...
          tgn.score as campaign_score,
          tgn.risk as campaign_risk
        from trust_graph_nodes tgn
  ...
  const campaignRows = campaignsRes.rows as PlayerCampaignRow[]
  ...
  const campaigns: CampaignViewRow[] = campaignRows.map((campaign) => ({
    ...campaign,
    normalized_risk: normalizeRisk(campaign.campaign_risk),
    normalized_score: clampScore(campaign.campaign_score)
  }))
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold">Campanhas vinculadas</h2>

            <p className="text-sm text-gray-500">
              Campanhas observadas na event_chain para este player.
            </p>
          </div>
        </div>
        ...
        {campaigns.length === 0 ? (
          <div className="mt-4 rounded-xl border border-dashed p-4 text-sm text-gray-500">
            Nenhuma campanha encontrada para este player.
          </div>
        ) : (
            ...
            <table className="min-w-full border-separate border-spacing-y-2">
              ...
              <tbody>
                {campaigns.map((campaign) => (
                  <tr key={campaign.campaign_id} className="bg-gray-50 text-sm">
                    ...
                    <td className="px-3 py-3 align-top">
                      {campaign.invalid_events}
                    </td>

                    <td className="px-3 py-3 align-top">
                      {formatDate(campaign.last_event_at)}
                    </td>

                    <td className="rounded-r-xl px-3 py-3 align-top">
                      <div className="flex flex-wrap gap-2">
                        <Link
                          href={`/proof?campaign_id=${encodeURIComponent(
                            campaign.campaign_id
                          )}`}
                        ...
                        <Link
                          href={`/audit/campaign/${encodeURIComponent(
                            campaign.campaign_id
                          )}`}
                          className="rounded-lg border px-3 py-1.5 text-xs font-medium hover:bg-white"
                        >
