...
function normalizeStatus(
  ...
  return Date.now() - time <= 2 * 60 * 1000 ? "online" : "offline"
}
...
export async function GET() {
  try {
    const res = await pool.query(`
      ...
      FROM all_players ap
      LEFT JOIN players_base pb
        ON pb.player_id = ap.player_id
      LEFT JOIN latest_heartbeat lh
        ON lh.player_id = ap.player_id
      LEFT JOIN player_metrics pm
        ON pm.player_id = ap.player_id
      LEFT JOIN graph_nodes gn
        ON gn.player_id = ap.player_id
    ...
    const withCoordinates = items.filter((item) =>
    ...
    const response: NetworkMapResponse = {
      ...
      summary: {
        ...
        watch: items.filter((item) => item.risk === "WATCH").length,
        highRisk: items.filter((item) => item.risk === "HIGH_RISK").length,
        withCoordinates,
