...
function isCandidateExecution(
  value: CandidateExecution | null
): value is CandidateExecution {
  return value !== null
}
...
function buildLinks(
  ...
  const eventId =
    typeof metaRecord.entity_id === "string" &&
    metaRecord.entity_type === "event"
      ? metaRecord.entity_id
      : null

  const blockId =
    typeof metaRecord.entity_id === "string" &&
    metaRecord.entity_type === "block"
      ? metaRecord.entity_id
      : null
  ...
  return {
    ...
    explorer_event_url: eventId
      ? `${origin}/explorer/event/${eventId}`
      : null,

    explorer_block_url: blockId
      ? `${origin}/explorer/block/${blockId}`
      : null,
...
async function safeGetCachedProof(
  key: string
): Promise<ProofResultLike | null> {
  try {
    return (await getCachedProof(
      key
    )) as ProofResultLike | null
  } catch (error) {
...
async function safeSetCachedProof(
  key: string,
  value: ProofResultLike
): Promise<void> {
  try {
...
async function tryCacheCandidates(
  ...
  > = await Promise.all(
    resolvedCandidates.map(
      async (
        candidate
      ): Promise<CandidateExecution | null> => {
        const cached =
          await safeGetCachedProof(
            candidate.cacheKey
          )
  ...
  > = await Promise.all(
    resolvedCandidates.map(
      async (
        ...
        try {
          const result =
            await runProofEngine(
              candidate.input
            )

          await safeSetCachedProof(
            candidate.cacheKey,
            result
          )
          ...
          console.warn(
            "VERIFY_HASH_LIVE_CANDIDATE_FAIL",
            {
              entity_id:
                candidate.input.entity_id,
    ...
    return jsonResponse(
      {
        ...
        message:
          "Proof verification queued for asynchronous processing",
