...
function TabCampanhas({ code, campaigns, onRefresh }: { code: string; campaigns: any[]; onRefresh: () => void }) {
  ...
  const [form, setForm] = useState({ name: "", startDate: "", endDate: "", budget: "", screens: [] as string[] });

  const toggleScreen = (id: string) =>
    setForm(f => ({ ...f, screens: f.screens.includes(id) ? f.screens.filter(s => s !== id) : [...f.screens, id] }));

  const handleSubmit = async () => {
    if (!form.name || !form.startDate || !form.endDate) { setErro("Preencha nome e datas."); return; }
    ...
    try {
      ...
      setErro(err.message || "Erro ao criar campanha.");
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: C.textSub, marginTop: 2 }}>{campaigns.length} campanha{campaigns.length !== 1 ? "s" : ""}</div>
        </div>
        <button onClick={() => { setShowForm(v => !v); setErro(""); }} style={{ background: C.primary, color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
          + Nova Campanha
        </button>
      </div>
      ...
      {showForm && (
        <div style={{ background: C.surface, border: "1px solid " + C.primary + "44", borderRadius: 12, padding: 24, marginBottom: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            <div>
              <label style={{ display: "block", fontSize: 12, color: C.textSub, marginBottom: 6 }}>NOME *</label>
            ...
            <div>
              <label style={{ display: "block", fontSize: 12, color: C.textSub, marginBottom: 6 }}>INÍCIO *</label>
              <input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} style={inputStyle} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 12, color: C.textSub, marginBottom: 6 }}>TÉRMINO *</label>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 8 }}>
              {SCREENS.map(sc => {
                ...
                return (
                  <div key={sc.id} onClick={() => toggleScreen(sc.id)} style={{ padding: "10px 14px", borderRadius: 8, cursor: "pointer", border: "1px solid " + (sel ? C.primary : C.border), background: sel ? C.primary + "18" : "transparent", display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, border: "2px solid " + (sel ? C.primary : C.muted), background: sel ? C.primary : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          {erro && <div style={{ background: C.danger + "18", border: "1px solid " + C.danger + "44", borderRadius: 8, padding: "10px 14px", marginBottom: 14, fontSize: 13, color: C.danger }}>⚠️ {erro}</div>}
          <div style={{ display: "flex", gap: 12 }}>
            <button onClick={handleSubmit} disabled={loading} style={{ background: C.primary, color: "#fff", border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 14, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}>
              {loading ? "Salvando…" : "Criar Campanha"}
            </button>
            <button onClick={() => { setShowForm(false); setErro(""); }} style={{ background: "transparent", color: C.textSub, border: "1px solid " + C.border, borderRadius: 8, padding: "10px 20px", fontSize: 14, cursor: "pointer" }}>
              Cancelar
            </button>
          </div>
        </div>
      ...
      {campaigns.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {campaigns.map((c: any) => (
            <div key={c.id} style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 12, padding: "18px 24px", display: "grid", gridTemplateColumns: "1fr auto auto auto auto", alignItems: "center", gap: 24 }}>
              <div>
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div
          ...
          style={{ border: "2px dashed " + (dragOver ? C.primary : C.border), borderRadius: 12, padding: "48px 24px", textAlign: "center", background: dragOver ? C.primary + "0A" : C.surface, cursor: "pointer" }}
        >
          <div style={{ fontSize: 36, marginBottom: 12 }}>📁</div>
          <div style={{ color: C.text, fontWeight: 600, marginBottom: 4 }}>{uploading ? "Enviando…" : "Arraste arquivos ou clique para selecionar"}</div>
...
function TabRelatorios({ campaigns }: { campaigns: any[] }) {
  ...
  const days = Array.from({ length: 7 }, (_, i) => {
    ...
    return { label: d.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }), value: Math.floor(Math.random() * 2000 + 500) };
  });
    ...
    <div>
      ...
      {campaigns.length > 0 && (
        <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 12, padding: 24 }}>
          ...
          <select value={selected} onChange={e => setSelected(e.target.value)} style={{ ...inputStyle, marginBottom: 16 }}>
            {campaigns.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          {campaign && (
...
function PortalAnuncianteInner({ code }: { code: string }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`* { box-sizing: border-box; margin: 0; padding: 0; } input, select { color-scheme: dark; } input::placeholder { color: #6B7280; }`}</style>
      ...
      <div style={{ borderBottom: "1px solid " + C.border, background: C.surface }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", gap: 4 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ background: "transparent", border: "none", cursor: "pointer", padding: "14px 20px", fontSize: 14, fontWeight: tab === t.id ? 600 : 400, color: tab === t.id ? C.primary : C.textSub, borderBottom: "2px solid " + (tab === t.id ? C.primary : "transparent"), display: "flex", alignItems: "center", gap: 8 }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
...
// ─── Page — sem use(params), sem Suspense wrapper complexo ────────────────────
export default function Page({ params }: { params: { code: string } }) {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#0B1020", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ color: "#9CA3AF" }}>Carregando…</div></div>}>
