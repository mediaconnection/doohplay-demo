...
export async function GET(req: NextRequest) {
  const code = req.url.split("/").slice(-1)[0]?.toUpperCase()
  ...
  try {
    ...
    const summary = {
      ...
      revenue_mrr: clients.reduce((a, c) => a + Number(c.revenue_month || 0), 0),
      plays_today: clients.reduce((a, c) => a + Number(c.plays_today || 0), 0),
      plays_month: clients.reduce((a, c) => a + Number(c.plays_month || 0), 0),
    }
