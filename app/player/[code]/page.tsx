    ...
    try {
      await fetch("/api/studio/playlist", {
        ...
        body: JSON.stringify({
          code,
          item_id: itemId,
          starts_at: dateForm.starts_at || null,
          ends_at: dateForm.ends_at || null,
        })
      })
      setPlaylistItems(prev => prev.map(i =>
        i.id === itemId
          ? { ...i, starts_at: dateForm.starts_at || null, ends_at: dateForm.ends_at || null }
          : i
      ))
              ...
              {playlistItems.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        setEditingDates(editingDates === item.id ? null : item.id)
                        setDateForm({
                      ...
                      style={{
                        background: (item.starts_at || item.ends_at) ? "#F0F9FF" : "#f9fafb",
                        border: `0.5px solid ${(item.starts_at || item.ends_at) ? "#BAE6FD" : "#e5e7eb"}`,
                        borderRadius: 8, padding: "6px 10px", fontSize: 12,
                        color: (item.starts_at || item.ends_at) ? "#0284C7" : "#9ca3af",
                  ...
                  {editingDates === item.id && (
                    <div style={{ marginTop: 10, padding: "12px 16px", background: "#F0F9FF", borderRadius: 10, border: "0.5px solid #BAE6FD" }}>
                      <div style={{ fontSize: 11, color: "#0369A1", fontWeight: 600, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                        Validade do item
                      </div>
                      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e => setDateForm(f => ({ ...f, starts_at: e.target.value }))}
                            style={{ background: "#fff", border: "0.5px solid #BAE6FD", borderRadius: 8, padding: "7px 10px", fontSize: 12, color: "#111", outline: "none" }}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type="datetime-local"
                            value={dateForm.ends_at}
                            onChange={e => setDateForm(f => ({ ...f, ends_at: e.target.value }))}
                            style={{ background: "#fff", border: "0.5px solid #BAE6FD", borderRadius: 8, padding: "7px 10px", fontSize: 12, color: "#111", outline: "none" }}
                          />
                        </div>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button
                            onClick={() => saveDates(item.id)}
                            disabled={savingDates}
                            style={{ background: "#0284C7", color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}
                          >
                            {savingDates ? "Salvando..." : "Salvar"}
                          </button>
                          <button
                            onClick={() => setDateForm({ starts_at: "", ends_at: "" })}
                            style={{ background: "#fff", color: "#6b7280", border: "0.5px solid #e5e7eb", borderRadius: 8, padding: "8px 12px", fontSize: 12, cursor: "pointer" }}
                          >
                            Limpar
                          </button>
                        </div>
