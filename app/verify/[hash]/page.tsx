...
export default function VerifyPage() {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const res = await fetch(`/api/proof-pdf/${hash}`)
      if (!res.ok) throw new Error("Erro ao gerar PDF")
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = `prova-doohplay-${hash.slice(0, 12)}.pdf`
      a.click()
  ...
  const verifyUrl = typeof window !== "undefined"
    ...
    : `https://doohplay-demo.onrender.com/verify/${hash}`
  const txHash = normalized.anchor_tx_hash
  const polygonUrl = data.links?.polygon_url
    ?? (txHash ? `https://polygonscan.com/tx/${txHash}` : null)
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-[1fr_auto_auto]">
            ...
            {/* QR Code */}
            <div className="rounded-2xl border border-slate-200 bg-white p-3 flex flex-col items-center gap-1">
              <QRCode
                value={verifyUrl}
                size={80}
                level="M"
          ...
          <div className="mt-5 flex flex-wrap gap-3 text-sm">
            ...
            <button
              ...
              className="rounded-xl border border-slate-200 px-4 py-2 font-medium text-slate-700 transition hover:bg-slate-50 disabled:opacity-50"
            >
              {downloadingPdf ? "Gerando PDF..." : "📄 Baixar prova em PDF"}
            </button>
            {polygonUrl ? (
              <a
                href={polygonUrl}
                target="_blank"
                rel="noreferrer"
                className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 font-medium text-emerald-700 transition hover:bg-emerald-100"
              >
                ⬡ Ver na Polygon
              </a>
            ) : null}
