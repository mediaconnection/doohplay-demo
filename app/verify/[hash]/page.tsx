...
function normalizeTrustScore(
  ...
  return {
    final_score:
      typeof trustScore.final_score === "number" &&
      Number.isFinite(trustScore.final_score)
        ? trustScore.final_score
        : 0,
    trust_level:
      typeof trustScore.trust_level === "string"
        ? trustScore.trust_level
        : "LOW",
    methodology:
      typeof trustScore.methodology === "string"
        ? trustScore.methodology
        : "verify-enterprise-v3",
    ...
    summary:
      typeof trustScore.summary === "string"
        ? trustScore.summary
...
function normalizeUiResult(data: VerifyResponse): VerificationResult {
  ...
  const technicalDetails =
    ...
    typeof data.explanation.technical_details === "object"
      ? data.explanation.technical_details
      : undefined
  ...
  const chain = details?.chain ?? {
    ...
    previous_event_hash:
      details?.chain?.previous_event_hash ??
      data.chain?.previous_event_hash ??
      data.event?.previous_event_hash ??
      null,
  ...
  const merkle = details?.merkle ?? {
    ...
    merkle_root:
      details?.merkle?.merkle_root ??
      details?.block_merkle_root ??
      data.merkle?.merkle_root ??
      data.event?.merkle_root ??
      null,
    proof_present:
  ...
  const blockchain = details?.blockchain ?? {
    ...
    tx_hash:
      details?.blockchain?.tx_hash ??
      details?.blockchain_tx ??
      details?.block_tx_hash ??
    ...
    tx_hash_format_valid:
      details?.blockchain?.tx_hash_format_valid ??
      details?.tx_valid ??
  ...
  const timestamp = details?.timestamp ?? {
    token_present:
      details?.timestamp?.token_present ??
      details?.timestamp_token_present ??
      data.tsa?.valid ??
      data.block?.timestamp_token_present ??
      data.evidence?.timestamp_token ??
      undefined
  }

  const execution = details?.execution ?? {
    event_id:
      details?.execution?.event_id ??
      details?.event_id ??
      data.event?.event_id ??
      null,
    created_at:
      details?.execution?.created_at ??
      details?.created_at ??
      data.event?.created_at ??
      null,
    block_created_at:
      details?.execution?.block_created_at ??
      data.block?.created_at ??
      null
  }
  ...
  const merkleIncluded =
    ...
    data.evidence?.merkle ??
    getBooleanRecordValue(technicalDetails, "merkle_included") ??
    getBooleanRecordValue(technicalDetails, "merkle") ??
    undefined

  const merkleProofValid =
    merkle?.proof_present ??
    getBooleanRecordValue(technicalDetails, "merkle_proof_valid") ??
    undefined

  const merkleRootValid =
    getBooleanRecordValue(technicalDetails, "merkle_root_valid") ??
  ...
  const chainValid =
    chain?.valid ??
    getBooleanRecordValue(technicalDetails, "chain_valid") ??
    undefined

  const chainHead =
    chain?.is_chain_head ??
    getBooleanRecordValue(technicalDetails, "chain_head") ??
    undefined

  const timestampValid =
    timestamp?.token_present ??
    getBooleanRecordValue(technicalDetails, "timestamp_valid") ??
    getBooleanRecordValue(technicalDetails, "timestamp_token_present") ??
    undefined
  ...
  const signaturePresent =
    ...
    getBooleanRecordValue(technicalDetails, "signature_present") ??
    undefined

  const signatureValid =
    details?.signature_valid ??
    data.verification?.signature_valid ??
    getBooleanRecordValue(technicalDetails, "signature_valid") ??
    undefined

  const blockchainValid =
    getBooleanRecordValue(technicalDetails, "blockchain_valid") ??
    getBooleanRecordValue(technicalDetails, "blockchain") ??
    data.blockchain?.confirmed ??
    data.blockchain?.anchored ??
  ...
  const normalizedTrustScore =
    normalizeTrustScore(data.trust_score) ??
    (typeof data.score === "number"
      ? {
          ...
          summary:
            ...
            data.summary ??
            (data.pending === true || data.status === "PENDING"
              ? "A validação foi enfileirada e ainda está em processamento."
              : "Sem resumo metodológico disponível.")
        }
      : undefined)

  const strengths =
    safeStringArray(data.strengths).length > 0
      ? safeStringArray(data.strengths)
      : normalizedTrustScore?.strengths ?? []

  const penalties =
    safeStringArray(data.penalties).length > 0
      ? safeStringArray(data.penalties)
      : normalizedTrustScore?.penalties ?? []

  const domains =
    Array.isArray(data.domains) && data.domains.length > 0
      ? data.domains
      : normalizedTrustScore?.domains ?? []
  ...
  const normalizedEvidence = {
    ledger:
      ...
      getBooleanRecordValue(technicalDetails, "block_exists") ??
      undefined,
    merkle:
      data.evidence?.merkle ??
    ...
    anchoring:
      data.evidence?.anchoring ??
      details?.anchored ??
      getBooleanRecordValue(technicalDetails, "anchored") ??
      undefined,
    ...
    source:
      ...
      data.source ??
      (typeof data.meta?.source === "string" ? data.meta.source : null)
  }

  const normalizedEventHash =
    data.hash_0x ??
    (typeof data.meta?.resolved_hash === "string"
      ? formatHash0x(data.meta.resolved_hash)
      : null) ??
    (typeof data.hash === "string" ? formatHash0x(data.hash) : null) ??
    (typeof data.meta?.hash === "string"
      ? formatHash0x(data.meta.hash)
      : null) ??
    (typeof data.meta?.input_hash === "string"
      ? formatHash0x(data.meta.input_hash)
      : null) ??
  ...
  return {
      ...
      score:
        typeof data.explanation?.score === "number"
          ...
          : typeof data.score === "number"
            ...
            : undefined,
      trust_level:
        data.explanation?.trust_level ??
        data.trust_level ??
        data.trust ??
        undefined,
      key_factors:
        safeStringArray(data.explanation?.key_factors).length > 0
          ? safeStringArray(data.explanation?.key_factors)
          : normalizedTrustScore?.strengths ?? [],
      risk_factors:
        safeStringArray(data.explanation?.risk_factors).length > 0
          ? safeStringArray(data.explanation?.risk_factors)
          : normalizedTrustScore?.penalties ?? [],
    ...
    metadata: {
      ...
      started_at:
        typeof data.meta?.started_at === "string" ? data.meta.started_at : undefined,
      finished_at:
        typeof data.meta?.finished_at === "string" ? data.meta.finished_at : undefined,
      duration_ms:
        typeof data.meta?.duration_ms === "number"
          ? data.meta.duration_ms
          : typeof data.meta?.execution_ms === "number"
            ? data.meta.execution_ms
            : undefined,
      execution_ms:
        typeof data.meta?.execution_ms === "number"
          ? data.meta.execution_ms
      ...
      request_id:
        typeof data.meta?.request_id === "string" ? data.meta.request_id : undefined,
      engine_version:
      ...
      source:
        data.source ??
        (typeof data.meta?.source === "string" ? data.meta.source : undefined),
      hash: typeof data.meta?.hash === "string" ? data.meta.hash : undefined,
      input_hash:
        typeof data.meta?.input_hash === "string" ? data.meta.input_hash : undefined,
      resolved_hash:
        typeof data.meta?.resolved_hash === "string"
          ? data.meta.resolved_hash
          : undefined,
      entity_id:
        typeof data.meta?.entity_id === "string" ? data.meta.entity_id : undefined,
      entity_type:
        typeof data.meta?.entity_type === "string" ? data.meta.entity_type : undefined,
      score:
        typeof data.score === "number"
          ? data.score
      ...
      confidence:
        typeof data.meta?.confidence === "number" ? data.meta.confidence : undefined,
      cache_hit:
        typeof data.meta?.cache_hit === "boolean" ? data.meta.cache_hit : undefined,
      queued:
        typeof data.meta?.queued === "boolean"
          ? data.meta.queued
          : data.pending === true
            ? true
            : undefined,
      certification_found:
        typeof data.meta?.certification_found === "boolean"
          ? data.meta.certification_found
          : undefined,
      certification_id:
        typeof data.meta?.certification_id === "string"
          ? data.meta.certification_id
          : data.meta?.certification_id === null
            ? null
            : undefined,
      evidence_sources:
        typeof data.meta?.evidence_sources === "object" && data.meta?.evidence_sources
          ? data.meta.evidence_sources
          : undefined,
      layers_enabled:
        typeof data.meta?.layers_enabled === "number"
          ? data.meta.layers_enabled
          : undefined,
      layers_executed:
        typeof data.meta?.layers_executed === "number"
          ? data.meta.layers_executed
          : undefined,
      enabled_layers: Array.isArray(data.meta?.enabled_layers)
        ? data.meta.enabled_layers
        : undefined,
      cross_layer_valid:
        typeof data.meta?.cross_layer_valid === "boolean"
          ? data.meta.cross_layer_valid
          : undefined,
      cross_layer_consistent:
        typeof data.meta?.cross_layer_consistent === "boolean"
          ? data.meta.cross_layer_consistent
          : undefined,
      cross_layer_summary:
        typeof data.meta?.cross_layer_summary === "string"
          ? data.meta.cross_layer_summary
          : undefined,
      cross_layer_issues: Array.isArray(data.meta?.cross_layer_issues)
        ? data.meta.cross_layer_issues
        : undefined
    },
    ...
    source:
      ...
      (typeof data.meta?.source === "string" ? data.meta.source : null),
    request_id:
      typeof data.meta?.request_id === "string" ? data.meta.request_id : null
  }
}
